//Øèôð Magma(ÃÎÑÒ 28147-89) Ïðîèçâîëüíûé òåêñò, 32 ðàóíäà, ñåòü ôåéñòåëÿ, íåëèíåéíîå ïðåîáðàçîâàíèå, ïîáèòîâûå öèêëè÷åñêèå ñäâèãè
...
//òàáëèöà çàìåí(íåëèíåéíîå áèåêòèâíîå ïðåîáðàçîâàíèå)
...
typedef uint8_t vect[4]; //áëîê ðàçìåðîì 32 áèòà

vect iter_key[32]; //èòåðàöèîííûå êëþ÷è øèôðîâàíèÿ

//äâîè÷íîå ïðåäñòàâëåíèå ñèìâîëà
...
//Ôóíêöèÿ ñëîæåíèÿ ïî ìîäóëþ 2
void GOST_Magma_XOR_2(uint8_t* a, uint8_t* b, uint8_t* c)
{
    for (int i = 0; i < 4; i++)
        c[i] = a[i] ^ b[i];
}
//Ôóíêöèÿ ñëîæåíèÿ ïî ìîäóëÿ 2 â ñòåïåíè 32
...
//Ôóíêöèÿ, âêëþ÷àþùàÿ ñëîæåíèå èòåðàöèîííîãî êëþ÷à è ëåâîãî ïîäáëîêà ïî ìîäóëþ 2 â ñòåïåíè 32, íåëèíåéíîå áèåêòèâíîå ïðåîáðàçîâàíèå è öèêëè÷åñêèé ñäâèã
...
//Ôóíêöèÿ ñëîæåíèÿ ðåçóëüòàòà g è ïðàâîãî ïîäáëîêà ïî ìîäóëþ 2 è èòîãîâîãî ôîðìèðîâàíèÿ íîâîãî ëåâîãî è ïðàâîãî ïîäáëîêà
...
//Ôóíêöèÿ äëÿ ðàçáèåíèÿ êëþ÷à íà âîñåìü ïîäêëþ÷åé
...
//Ôóíêöèÿ äëÿ îïðåäåëåíèå èòåðàöèîííûõ(ðàóíäîâûõ) êëþ÷åé
...
//Â ãëàâíîé ôóíêöèè ðåàëèçîâàíî øèôðîâàíèå è äåøèôðîâàíèå ñîîáùåíèÿ
int main()
{
    ...
    cout << "Äëèíà äîïîëíåííîãî ñîîáùåíèÿ:" << endl;
    cout << length << endl;
    cout << "Áèíàðíûé âèä ñîîáùåíèÿ:" << endl;
    ...
    error_mess[0] = 64;
    //Çàïîëíÿåì íóëåâûìè áàéòàìè ïîñëåäíèé áëîê
    if (length > size)
    {
        new_mess[size] = 0;
        hash_mess[size] = 128; // äëÿ õýø ôóíêöèè äîïîëíåíèå ïîñëåäíåãî áëîêà íà÷èíàåòñÿ ñ 10000000
        error_mess[size] = 128;
    ...
    cout << "Áèíàðíûé âèä äîïîëíåííîãî ñîîáùåíèÿ:" << endl;
    print_binary_mess(new_mess, length);
    cout << "Áèíàðíûé âèä èçìåííåíîãî ñîîáùåíèÿ íà 1 áèò:" << endl;
    ...
    printf("\n");
    cout << "Ïîëó÷åííîå õýø-çíà÷åíèå äëÿ äàííîãî ñîîáùåíèÿ:" << endl;
    for (int i = 0; i < 8; i++) printf("%02x", hash_value[i]);
    printf("\n");
    cout << "Ïîëó÷åííîå õýø-çíà÷åíèå äëÿ èçìåíåííîãî íà 1 áèò ñîîáùåíèÿ:" << endl;
